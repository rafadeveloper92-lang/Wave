...
export function useWebRtcCall(roomId: string, opts: { enabled: boolean; isGroup: boolean }) {
  ...
  const sendSignal = useCallback(async (payload: SignalPayload) => {
    ...
    if (sendErr) {
      ...
      setError('Falha ao enviar sinalização. Verifique o Supabase Realtime.');
    }
  }, []);
  ...
  const startCall = useCallback(
    async (kind: CallKind) => {
      if (!opts.enabled || opts.isGroup) {
        setError('Chamadas em grupo ainda não são suportadas.');
        return;
      }
      ...
      try {
        ...
        console.error(e);
        setError(e instanceof Error ? e.message : 'Não foi possível acessar microfone ou câmera.');
        cleanupMedia();
      }
    },
    [opts.enabled, opts.isGroup, attachPcHandlers, sendSignal, flushPendingIce, cleanupMedia]
  );
  ...
  const acceptCall = useCallback(async () => {
    ...
    try {
      ...
      console.error(e);
      setError(e instanceof Error ? e.message : 'Erro ao atender a chamada.');
      await endCall(true);
    }
  ...
  useEffect(() => {
    ...
    void ch.subscribe((status) => {
      if (status === 'CHANNEL_ERROR') {
        setError('Canal Realtime indisponível. Ative Broadcast no Supabase.');
      }
    });
  ...
  }, [opts.enabled, roomId, cleanupMedia, flushPendingIce]);
