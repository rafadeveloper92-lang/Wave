...
export default function AuthScreen() {
  ...
  const onSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (error) setMessage(error);
        ...
        setMessage('Conta criada. Se o projeto exigir confirmação por email, verifica a caixa de entrada; depois inicia sessão.');
      }
    } finally {
      setBusy(false);
    }
  ...
  return (
    <div className="min-h-screen flex flex-col items-center justify-center bg-[#020617] px-6 py-10 text-white font-sans">
      <motion.div
      ...
      >
        <div className="text-center space-y-1">
          ...
          <p className="text-xs text-gray-500">
            {mode === 'login'
              ? 'Inicia sessão para usar o chat ligado ao Supabase.'
              : 'Cria conta — os dados ficam no teu projeto Supabase.'}
          </p>
        </div>
